  ...
  def assign_arrow_angles
    Arrow.transaction do
      Arrow.includes(figure: { page: :image }).find_each do |arrow|
        image = ImageProcessing.extractFigure(arrow.figure, arrow.figure.page.image.data)
        ...
        index = rects.each_with_index.max_by do |r, i|
        ...
        row, index1 = distances.each_with_index.max_by { |row, index| row.max }
        dist, index2 = row.each_with_index.max_by { |d, index| d }
        ...
        if p1_distances > p2_distances
          arrow.angle = (arrow.angle + 180) % 360
        end
        arrow.save!
      end
    end
